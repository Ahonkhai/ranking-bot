...
def test_renders_at_the_expected_size():
    ...
    assert (img.width, img.height) == (760, 440)
...
def test_empty_board_still_renders():
    assert open_png(make_top3_image([])).width == 760
...
def test_ties_keep_their_shared_rank():
    ...
    assert open_png(make_top3_image(tied)).width == 760
...
def test_hostile_and_oversized_input():
    ...
    assert open_png(make_top3_image(rough)).width == 760


def test_corrupt_avatar_bytes_fall_back():
    avatars = {1: b"not an image", 2: None, 3: None}
    assert open_png(make_top3_image(TOP3, avatars)).width == 760
